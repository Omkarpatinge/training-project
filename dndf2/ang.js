var app = angular.module('app', ['dndLists','daterangepicker']);
...
app.filter('searching',function(){
	return function(input,value,objname,ftype) {	
		...
		if (objname&&ftype==='in') {
			...
			for (var i = 0; i < input.length; i++) {
				if(input[i].name.toLowerCase().indexOf(value.toLowerCase())>=0)
					filtered.push(input[i]);
			}
		}
		return filtered;
...
app.factory('callserver',function(urls,$http) {
	return {
        calldim: function() {
...
app.controller('MainCtrl', function($scope,$http,callserver){
    ...
    $scope.$watch('lists', function(lists,listsold) {
       ...
       /* for (i = 0; i < $scope.lists[2].people.length; i++) {	
        }*/
