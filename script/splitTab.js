...
define(['app', 'jquery', 'requestService', 'dataFactory'], function (app, $) {
	app.directive('splitTab', ['requestService', 'dataFactory', '$compile', function (requestService, dataFactory, $compile) {
		// Runs during compile	
		return {
			// name: '',
			// priority: 1,
			...
			template: '<tr>' + '<td>Total</td>' + '<td ng-repeat="m in metrics">{{result[m]|numberSuffix:2}}</td>' + '</tr>',
			// templateUrl: '',
			//replace: true,
			//transclude: true,
